  ...
  return (
    <DrawerShell side="right" width={320} onClose={onClose}>
      <div className="flex flex-shrink-0 items-center gap-2 border-b border-border px-4 py-3">
        <span className="flex-1 font-mono text-xs font-semibold text-text">Active Agents</span>
        ...
        <button
          ...
          onClick={onClose}
          aria-label="Close"
          className="ml-2 cursor-pointer font-mono text-sm text-muted hover:text-text"
        >
          ×
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {grouped.map(({ repo, agents }) => (
          <div key={repo.slug}>
            <div className="flex items-center gap-2 border-b border-border bg-surface-2 px-3.5 py-2">
              ...
              <span className="font-mono text-[11px] font-semibold text-text">{repo.name}</span>
            </div>
            {agents.map((a) => (
        ...
        {grouped.length === 0 && (
          <div className="p-6 text-center font-mono text-[10px] text-muted">no agent activity</div>
        )}
      </div>

      <div className="flex-shrink-0 border-t border-border px-3.5 py-2.5">
        <div className="text-center font-mono text-[10px] text-muted">click any agent to open its repo view</div>
      </div>
    </DrawerShell>
  )
}
  ...
  return (
    ...
    >
      <div className="mb-1 flex items-center gap-2">
        ...
        />
        <span className="flex-1 font-mono text-[10px] text-text">{agent.label}</span>
        <span className="font-mono text-[9px] text-muted">{elapsedLabel}</span>
      </div>
      <ProgressBar
        pct={agent.pct}
        ...
        className="ml-3.5"
      />
      <div className="mt-1 ml-3.5 flex items-center gap-1.5 font-mono text-[9px] text-muted">
