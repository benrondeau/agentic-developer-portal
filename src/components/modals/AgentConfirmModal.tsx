  ...
  return (
    <ModalShell onClose={onCancel}>
      ...
      <div className="px-4 py-4">
        ...
        <div className="rounded-[3px] border border-dashed border-border bg-surface-2 px-2.5 py-2 font-mono text-[12px] leading-relaxed text-muted">
...
function Cell({ label, value, sub }: { label: string; value: string; sub: string }) {
  return (
    <div className="rounded-[4px] border border-border bg-surface-2 px-2.5 py-2">
      <div className="mb-0.5 font-mono text-[11px] tracking-[0.06em] text-muted uppercase">{label}</div>
      <div className="font-mono text-[18px] font-semibold text-text">{value}</div>
      <div className="mt-px font-mono text-[11px] text-muted">{sub}</div>
    </div>
  )
}
